...
export default async function AiModelsPage({ searchParams }: { searchParams: Promise<{ useCase?: string | string[] }> }) {
  ...
  const useCase: AiUseCase = parsedUseCase.success ? parsedUseCase.data : 'AI_PASTOR';
  const overview = await getAiRuntimeOverview(useCase).catch(() => null);
  return (
    <div className="mx-auto max-w-7xl">
      <header className="mb-6 flex items-start gap-3">
        ...
        <div><p className="text-sm font-medium text-emerald-700">AI 운영</p><h1 className="mt-1 text-2xl font-bold text-slate-950">모델 설정</h1><p className="mt-2 text-sm text-slate-600">기능별 설정과 변경 이력을 분리해 한 기능의 변경이 다른 기능에 영향을 주지 않게 합니다.</p></div>
      </header>
...
function UseCaseTab({ href, active, children }: { href: string; active: boolean; children: React.ReactNode }) {
  return <Link href={href} aria-current={active ? 'page' : undefined} className={`border-b-2 px-4 py-3 text-sm font-semibold ${active ? 'border-emerald-700 text-emerald-800' : 'border-transparent text-slate-500 hover:text-slate-800'}`}>{children}</Link>;
}
