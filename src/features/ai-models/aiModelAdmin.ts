...
export const runtimeConfigSchema = z.object({
  id: z.string().uuid().nullable(), useCase: z.literal('AI_PASTOR'), version: z.number(),
...
export async function getAiRuntimeOverview(): Promise<AiRuntimeOverview> {
  const { accessToken } = await requireAdminSession();
  const response = await fetch(backendUrl('/api/v1/admin/ai/runtime-config'), {
