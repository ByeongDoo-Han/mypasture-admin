...
지정 계정의 운영 관리자 승격은 서버 작업에서 완료했습니다. 이메일 코드 기능은 아직 운영에 배포하지 않았습니다. Cloudflare Email Sending은 해당 계정에서 유료 가입을 요구했으며, 사용자는 가입을 보류했습니다. SMTP 인증 정보와 서버 활성화가 완료되기 전에 이 UI를 운영 배포하면 로그인할 수 없습니다.
