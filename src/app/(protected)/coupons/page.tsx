...
/** 목장 꾸미기 이벤트 보상 코드를 발급하고 등록률과 활성 상태를 운영하는 페이지입니다. */
export default async function CouponCampaignPage({ searchParams }: { searchParams: Promise<{ status?: string; page?: string }> }) {
  ...
  return <div className="mx-auto max-w-7xl"><header className="mb-6 flex items-start gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center bg-emerald-50 text-emerald-800"><TicketPercent size={20} /></span><div><p className="text-sm font-medium text-emerald-700">커뮤니티 운영</p><h1 className="mt-1 text-2xl font-bold text-slate-950">이벤트 보상 코드</h1><p className="mt-2 text-sm text-slate-600">유료 기능과 무관한 비구매 목장 꾸미기 아이템만 지급합니다.</p></div></header><nav aria-label="이벤트 보상 상태 필터" className="mb-5 flex border-b border-slate-200">{[[undefined, '전체'], ['ACTIVE', '활성'], ['INACTIVE', '중지']] .map(([value, label]) => <Link key={label} href={value ? `/coupons?status=${value}` : '/coupons'} className={`border-b-2 px-4 py-2 text-sm font-semibold ${status === value ? 'border-emerald-700 text-emerald-800' : 'border-transparent text-slate-500'}`}>{label}</Link>)}</nav>{data ? <><CouponCampaignOperations data={data} />{data.totalPages > 1 ? <nav aria-label="이벤트 보상 페이지 이동" className="mt-5 flex justify-end gap-2"><PageLink disabled={data.page === 0} href={`/coupons?${status ? `status=${status}&` : ''}page=${Math.max(0, data.page - 1)}`}>이전</PageLink><PageLink disabled={data.page + 1 >= data.totalPages} href={`/coupons?${status ? `status=${status}&` : ''}page=${data.page + 1}`}>다음</PageLink></nav> : null}</> : <div role="alert" className="border-l-4 border-amber-500 bg-amber-50 px-4 py-3 text-sm text-amber-900">이벤트 보상 운영 데이터를 불러오지 못했습니다. 백엔드 연결과 관리자 권한을 확인해 주세요.</div>}</div>;
}
