...
/** 쿠폰 캠페인 생성, 최초 코드 전달과 안전한 비활성화를 제공하는 운영 화면입니다. */
export function CouponCampaignOperations({ data }: { data: CouponCampaignPage }) {
  ...
  async function create(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      entitlementDurationDays: Number(form.get('durationDays')), aiDailyLimit: Number(form.get('aiDailyLimit')), reason: String(form.get('reason') ?? ''),
    };
    ...
    setGeneratedCodes(result?.generatedCodes ?? []); setMessage({ kind: 'success', text: '쿠폰 캠페인을 생성했습니다. 코드는 지금 한 번만 안전하게 보관해 주세요.' });
  ...
  async function deactivate(id: string, name: string) {
    ...
    if (!window.confirm('이미 지급된 체험 권한은 유지되고 신규 등록만 중지됩니다. 계속할까요?')) return;
    ...
    <div className="space-y-8">
      <section className="border-t border-slate-200 pt-5">
        ...
        <form onSubmit={create} className="grid gap-4 lg:grid-cols-4">
          <Field label="캠페인 이름"><input required name="name" minLength={2} maxLength={100} className={inputClass} placeholder="첫 가입자 7일 체험" /></Field>
          <Field label="체험 자격 키"><input required name="eligibilityKey" minLength={2} maxLength={80} className={inputClass} defaultValue="premium-ai-trial-lifetime" /></Field>
          ...
          {codeType === 'SHARED' ? <><Field label="공유 코드 (비우면 자동 생성)"><input name="sharedCode" minLength={8} maxLength={40} className={inputClass} placeholder="PASTURE-TRIAL-2026" /></Field><Field label="전체 등록 한도"><input required name="maxRedemptions" type="number" min={1} max={1_000_000} defaultValue={100} className={inputClass} /></Field></> : <Field label="개별 코드 수량"><input required name="codeCount" type="number" min={1} max={1000} defaultValue={100} className={inputClass} /></Field>}
          <Field label="체험 기간(일)"><input required name="durationDays" type="number" min={1} max={90} defaultValue={7} className={inputClass} /></Field>
          <Field label="AI 일일 한도"><input required name="aiDailyLimit" type="number" min={1} max={500} defaultValue={30} className={inputClass} /></Field>
