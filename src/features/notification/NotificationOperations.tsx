...
export function NotificationOperationsView({ initial }: { initial: NotificationOperations }) {
  ...
  return <div className="space-y-6">
    <section aria-label="알림 핵심 지표" className="grid border-y border-slate-200 bg-white sm:grid-cols-2 xl:grid-cols-4 xl:divide-x xl:divide-slate-200">
      ...
      <Metric label="앱 열람" value={summary.opened} detail="인앱에서 읽음 처리된 수" />
    ...
    <div className="flex items-start gap-2 border-l-4 border-blue-600 bg-blue-50 px-3 py-3 text-sm text-blue-950"><BellRing size={17} className="mt-0.5 shrink-0" /><p>Provider 접수는 FCM이 요청을 수락했다는 뜻이며 실제 기기 도착을 보장하지 않습니다. 앱 열람은 사용자가 인앱 알림을 연 기록입니다.</p></div>
    ...
    <section>
      ...
      <div className="overflow-x-auto border-y border-slate-200 bg-white"><table className="w-full min-w-[900px] text-left text-sm"><thead className="bg-slate-50 text-xs text-slate-500"><tr><Th>게시</Th><Th>유형</Th><Th>내용</Th><Th>기기 작업</Th><Th>Provider 접수</Th><Th>최종 실패</Th><Th>앱 열람</Th></tr></thead><tbody className="divide-y divide-slate-100">{operations.broadcasts.map((broadcast) => <tr key={broadcast.id}><Td>{formatDate(broadcast.publishedAt)}</Td><Td><Status>오늘의 말씀</Status></Td><Td><strong className="block text-slate-800">{broadcast.title}</strong><span className="mt-1 block text-xs text-slate-500">{broadcast.body}</span></Td><Td>{broadcast.deviceDeliveries.toLocaleString('ko-KR')}</Td><Td>{broadcast.sentToProvider.toLocaleString('ko-KR')}</Td><Td><span className={broadcast.dead > 0 ? 'font-semibold text-red-700' : ''}>{broadcast.dead.toLocaleString('ko-KR')}</span></Td><Td>{broadcast.opened.toLocaleString('ko-KR')}</Td></tr>)}{operations.broadcasts.length === 0 ? <tr><td colSpan={7} className="px-4 py-10 text-center text-slate-500">조건에 맞는 알림이 없습니다.</td></tr> : null}</tbody></table></div>
